...
public class TextAnalyzer {
  ...
  public void showDirectedGraph() {
    ...
    graph.setAttribute("ui.stylesheet", "node{ size: 10px, 10px; shape: circle; fill-color: "
            +
            "#B1DFF7; stroke-mode: plain; stroke-color: #B1DFF7; stroke-width: 5; text-mode: "
            +
            "normal; text-size: 20px; /*normal or hidden*/ text-background-mode: plain; /*plain "
            + "or none*/ text-background-color: rgba(255, 255, 255, 200); text-alignment: above;}"
            + "edge { "
            + "   text-size: 20px; "
            +  // 设置边标签字体大小
            "}");
  ...
  public List<String> my_Dij(String start, String end) {
    ...
    PriorityQueue<NodeDistance> pq =
            new PriorityQueue<>(Comparator.comparingInt(nd -> nd.distance));
    //是否访问过
    ...
    for (Map.Entry<String, Integer> entry : dist.entrySet()) {
      System.out.println("Distance from " + start + " -> " + entry.getKey()
              + " is " + entry.getValue());
    }
    return path;
  }
        ...
        if (fun.equals("3")) {
          ...
          if (bridges != null && bridges.equals(list)) {
            ...
            if (bridges.size() == 0) {
              ...
              System.out.println("The bridge words from \"" + words[0] + "\" to \"" + words[1] +
